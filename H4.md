...
## Aloitus 
Tällä kertaa tehtävien tarkoituksena oli käsitellä oma tekemää komentoa, joka luodaan ensiksi, testataan yhdellä koneella toimivaksi ja lopulta automatisoidaan se jakamalla komento orjakoneille. Aloitin tehtävät 19.4.2023 klo 16:00 ajamalla komennon ```vagrant up``` käynnistämällä koneet ```tmaster```, ```t001``` ja ```t002```.
...
## Lopetus
Lopetin tehtävien teon klo 17:41. Kyseinen tehtävä syvensi Saltin käytön eri vaihtoehtoja ja tehtävässä pääsi myös vähän soveltamaankin uusia asioita. Töihin meni kokonaisuudessaan tällä erää n. 2h.
