...
## Lopetus
Lopetin tehtävien teon klo 17:41 kirjatumalla isäntäkoneen SSH-yhteydestä komennolla ```exit``` ja sammuttalla sekä herra että orja koneet komennolla ```vagrant halt```. Kyseinen tehtävä syvensi Saltin käytön eri vaihtoehtoja ja tehtävässä pääsi myös vähän soveltamaan uusiakin asioita. Töihin meni kokonaisuudessaan tällä erää n. 2h.
